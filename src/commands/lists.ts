...
export async function lpush(
  ...
  return await (client as any).glideClient.lpush(
    normalizedKey,
    normalizedValues
  );
}
...
export async function rpush(
  ...
  const normalizedValues = values.map(ParameterTranslator.normalizeValue);
  return await (client as any).glideClient.rpush(
    normalizedKey,
    normalizedValues
  );
}
...
export async function lpop(
  ...
  if (count !== undefined) {
    const results = await (client as any).glideClient.lpopCount(
      normalizedKey,
      count
    );
    return results
...
export async function rpop(
  ...
  if (count !== undefined) {
    const results = await (client as any).glideClient.rpopCount(
      normalizedKey,
      count
    );
    return results
...
export async function lrange(
  ...
  await (client as any).ensureConnection();
  const normalizedKey = (client as any).normalizeKey(key);
  const results = await (client as any).glideClient.lrange(
    normalizedKey,
    start,
    stop
  );
...
export async function lrem(
  ...
  const normalizedKey = (client as any).normalizeKey(key);
  const normalizedValue = ParameterTranslator.normalizeValue(value);
  return await (client as any).glideClient.lrem(
    normalizedKey,
    count,
    normalizedValue
  );
