...
export async function hget(client: BaseClient, key: RedisKey, field: string): Promise<string | null> {
...
export async function hset(client: BaseClient, key: RedisKey, ...args: any[]): Promise<number> {
  const normalizedKey = (client as any).normalizeKey(key);
...
export async function hgetall(client: BaseClient, key: RedisKey): Promise<Record<string, string>> {
  const normalizedKey = (client as any).normalizeKey(key);
  ...
  if (Array.isArray(result)) {
    ...
    for (const item of result) {
      if (item && typeof item === 'object' && 'field' in item && 'value' in item) {
        const field = ParameterTranslator.convertGlideString((item as any).field) || '';
        const value = ParameterTranslator.convertGlideString((item as any).value) || '';
...
export async function hmset(client: BaseClient, key: RedisKey, ...args: any[]): Promise<string> {
  const normalizedKey = (client as any).normalizeKey(key);
  const fieldValuePairs: Record<string, string> = {};
  if (args.length === 1 && typeof args[0] === 'object' && !Array.isArray(args[0])) {
    const obj = args[0];
...
export async function hmget(client: BaseClient, key: RedisKey, ...fieldsOrArray: any[]): Promise<(string | null)[]> {
  const normalizedKey = (client as any).normalizeKey(key);
  const fields = Array.isArray(fieldsOrArray[0]) ? fieldsOrArray[0] : fieldsOrArray;
  const results = await (client as any).glideClient.hmget(normalizedKey, fields);
  return results.map(ParameterTranslator.convertGlideString);
}

export async function hdel(client: BaseClient, key: RedisKey, ...fields: string[]): Promise<number> {
  const normalizedKey = (client as any).normalizeKey(key);
  return await (client as any).glideClient.hdel(normalizedKey, fields);
}

export async function hexists(client: BaseClient, key: RedisKey, field: string): Promise<number> {
  const normalizedKey = (client as any).normalizeKey(key);
  const result = await (client as any).glideClient.hexists(normalizedKey, field);
  return result ? 1 : 0;
}

export async function hkeys(client: BaseClient, key: RedisKey): Promise<string[]> {
  const normalizedKey = (client as any).normalizeKey(key);
...
export async function hvals(client: BaseClient, key: RedisKey): Promise<string[]> {
  const normalizedKey = (client as any).normalizeKey(key);
  const results = await (client as any).glideClient.hvals(normalizedKey);
  return results.map((r: any) => ParameterTranslator.convertGlideString(r) || '');
}

export async function hstrlen(client: BaseClient, key: RedisKey, field: string): Promise<number> {
  const normalizedKey = (client as any).normalizeKey(key);
...
export async function hrandfield(
  ...
  return fields.map((f: any) => ParameterTranslator.convertGlideString(f) || '');
}
...
export async function hincrby(
  ...
  const normalizedKey = (client as any).normalizeKey(key);
  return await (client as any).glideClient.hincrBy(normalizedKey, field, increment);
}
