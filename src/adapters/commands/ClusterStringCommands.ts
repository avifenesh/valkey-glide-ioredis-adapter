/**
 * Cluster String Commands - Redis string operations for cluster mode
 */
...
export class ClusterStringCommands {
  constructor(private getClient: () => Promise<GlideClusterClient>) {}
  ...
  async set(key: RedisKey, value: RedisValue, ...args: any[]): Promise<string | null> {
    ...
    for (let i = 0; i < args.length; i += 2) {
      ...
      if (option === 'EX') options.expiry = { type: 'seconds', count: Number(optionValue) };
      else if (option === 'PX') options.expiry = { type: 'milliseconds', count: Number(optionValue) };
  ...
  async setex(key: RedisKey, seconds: number, value: RedisValue): Promise<string> {
    ...
    const normalizedKey = ParameterTranslator.normalizeKey(key);
    const normalizedValue = ParameterTranslator.normalizeValue(value);
    await client.customCommand(['SETEX', normalizedKey, seconds.toString(), normalizedValue]);
    return 'OK';
  }
  ...
  async psetex(key: RedisKey, milliseconds: number, value: RedisValue): Promise<string> {
    ...
    const normalizedKey = ParameterTranslator.normalizeKey(key);
    const normalizedValue = ParameterTranslator.normalizeValue(value);
    await client.customCommand(['PSETEX', normalizedKey, milliseconds.toString(), normalizedValue]);
    return 'OK';
  }
}
