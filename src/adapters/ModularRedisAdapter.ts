/**
 * Modular RedisAdapter Implementation
 * ioredis-compatible client built on valkey-glide with modular command structure
 */

import { BaseRedisAdapter } from './BaseRedisAdapter';
import { StringCommands } from './commands/StringCommands';
...
export class ModularRedisAdapter extends BaseRedisAdapter {
  ...
  constructor(options: RedisOptions = {}) {
    ...
    this.stringCommands = new StringCommands(() => this.ensureConnected());
    this.listCommands = new ListCommands(() => this.ensureConnected());
    this.hashCommands = new HashCommands(() => this.ensureConnected());
    this.zsetCommands = new ZSetCommands(() => this.ensureConnected());
    this.pubsubCommands = new PubSubCommands(() => this.ensureConnected(), this.options);
    this.transactionCommands = new TransactionCommands(() => this.ensureConnected());
  ...
  static createClient(type: 'client' | 'subscriber' | 'bclient', options?: RedisOptions): ModularRedisAdapter {
    const adapter = new ModularRedisAdapter(options || {});
  ...
  async duplicate(override?: Partial<RedisOptions>): Promise<ModularRedisAdapter> {
  ...
  pipeline(): Pipeline {
    // TODO: Implement proper pipeline adapter for modular architecture
    return new PipelineAdapter(this as any);
  }

  multi(): Multi {
    // TODO: Implement proper multi adapter for modular architecture
