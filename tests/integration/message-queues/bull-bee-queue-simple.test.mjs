...
import { describe, it, test, beforeEach, afterEach, after } from 'node:test';
...
describe('Message Queue Systems Integration (No Workers)', () => {
  ...
  describe('Bull Queue - No Workers', () => {
        ...
        try {
          ...
          await Promise.race([
            ...
            new Promise((_, reject) => {
              const t = setTimeout(
                () => reject(new Error('Queue close timeout')),
                5000
              );
              t.unref?.();
            }),
    ...
    test('should support job priorities', async () => {
      ...
      const highPriority = await queue.add(
        'priority-job',
        { priority: 'high' },
        { priority: 10 }
      );
      const lowPriority = await queue.add(
        'priority-job',
        { priority: 'low' },
        { priority: 1 }
      );
      const mediumPriority = await queue.add(
        'priority-job',
        { priority: 'medium' },
        { priority: 5 }
      );
