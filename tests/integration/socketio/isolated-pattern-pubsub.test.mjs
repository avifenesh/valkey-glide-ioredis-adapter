...
describe('Isolated Pattern Pub/Sub Test', () => {
  ...
  test('Pattern subscription works with IoredisPubSubClient', async () => {
    ...
    subClient.on(
      'pmessage',
      (pattern, channel, message | Buffer) => {
    ...
    assert.strictEqual(receivedPatternMessages[0]?.channel, 'socket.io#/#general#');
    assert.ok(receivedPatternMessages[0]?.message.includes(
    ...
    assert.strictEqual(receivedBufferMessages[0]?.channel, 'socket.io#/#general#');
