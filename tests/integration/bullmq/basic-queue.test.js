...
describe('BullMQ Integration - Basic Queue Operations', () => {
    ...
    beforeAll(async () => {
        serversAvailable = await setup_1.testUtils.checkTestServers();
        if (!serversAvailable) {
    ...
    const itConditional = (name, testFn) => {
        it(name, async () => {
            // Check server availability at runtime for each test
            const currentServerStatus = await setup_1.testUtils.checkTestServers();
    ...
    beforeEach(async () => {
        ...
        const healthCheck = await setup_1.testUtils.checkTestServers();
        if (!healthCheck) {
        ...
        redisAdapter = new RedisAdapter_1.RedisAdapter(config);
        ...
        queue = new bullmq_1.Queue(testQueueName, {
        ...
        processedJobs = [];
        worker = new bullmq_1.Worker(testQueueName, async (job) => {
            processedJobs.push({
                id: job.id,
                data: job.data,
                name: job.name,
        ...
        const workerTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Worker ready timeout')), 10000));
        try {
    ...
    afterEach(async () => {
        ...
        if (worker) {
            cleanupPromises.push(worker.close().catch(err => console.warn('Worker close error:', err.message)));
        }
        if (queue) {
            cleanupPromises.push(queue.close().catch(err => console.warn('Queue close error:', err.message)));
        }
        // Wait for BullMQ cleanup
        await Promise.allSettled(cleanupPromises);
        await setup_1.testUtils.delay(200); // Increased delay for thorough cleanup
    ...
    describe('Basic Job Processing', () => {
        itConditional('should add and process a simple job', async () => {
            ...
            await setup_1.testUtils.delay(500);
        ...
        itConditional('should handle job with priority', async () => {
            ...
            await setup_1.testUtils.delay(2000);
    ...
    describe('Job State Management', () => {
        itConditional('should track job states correctly', async () => {
            ...
            const freshJob = await bullmq_1.Job.fromId(queue, job.id);
        ...
        itConditional('should handle job removal', async () => {
            ...
            await setup_1.testUtils.delay(400);
    ...
    describe('Queue Statistics', () => {
        itConditional('should provide accurate queue counts', async () => {
            ...
            // Wait for processing
            await setup_1.testUtils.delay(800);
    ...
    describe('Error Handling', () => {
        itConditional('should handle job failures gracefully', async () => {
            ...
            const failWorker = new bullmq_1.Worker(testQueueName, async (job) => {
        ...
        itConditional('should handle connection recovery scenarios', async () => {
            ...
            await setup_1.testUtils.delay(200);
        ...
        itConditional('should provide meaningful error messages for setup failures', async () => {
            ...
            try {
                ...
                const invalidQueue = new bullmq_1.Queue('invalid-test-queue', {
        ...
        itConditional('should handle worker initialization errors gracefully', async () => {
            ...
            try {
                ...
                testWorker = new bullmq_1.Worker(testQueueName, async (job) => {
                ...
                const freshJob = await bullmq_1.Job.fromId(queue, errorJob.id);
                const state = await freshJob.getState();
                expect(['failed', 'stalled', 'completed']).toContain(state);
            }
            finally {
...
//# sourceMappingURL=basic-queue.test.js.map