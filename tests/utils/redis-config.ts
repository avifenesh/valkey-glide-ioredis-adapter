...
export async function getRedisTestConfig(): Promise<RedisTestConfig> {
  ...
  const valkeyBundlePort = process.env.VALKEY_BUNDLE_PORT
    ? Number(process.env.VALKEY_BUNDLE_PORT)
    : undefined;

  if (valkeyBundleHost && valkeyBundlePort) {
    console.log(
      `Using CI Valkey Bundle server: ${valkeyBundleHost}:${valkeyBundlePort}`
    );
  ...
  const valkeyPort = process.env.VALKEY_STANDALONE_PORT
    ? Number(process.env.VALKEY_STANDALONE_PORT)
    : undefined;
  ...
  if (envHost && envPort) {
    console.log(`Using Valkey test server: ${envHost}:${envPort}`);
    return { host: envHost, port: envPort };
  }

  // AVOID discovery as it might find old cluster nodes - use safe default
  console.warn(
    'No explicit test server config found, using Valkey default localhost:6379'
  );
