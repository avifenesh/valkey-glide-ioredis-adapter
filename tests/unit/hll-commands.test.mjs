...
describe('HyperLogLog Commands', () => {
  ...
  describe('PFADD', () => {
    ...
    it('should add multiple elements', async () => {
      const result = await client.pfadd(testKey, 'element1', 'element2', 'element3');
    ...
    it('should handle mixed new and existing elements', async () => {
      ...
      const result = await client.pfadd(testKey, 'element2', 'element3', 'element4');
    ...
    it('should handle special characters', async () => {
      const result = await client.pfadd(testKey, 'user@example.com', 'user#123', 'user$456');
      assert.strictEqual(result, 1);
    });

    it('should handle Unicode characters', async () => {
      const result = await client.pfadd(testKey, '用户1', 'ユーザー2', 'مستخدم3');
      assert.strictEqual(result, 1);
    });
  });
  ...
  describe('PFCOUNT', () => {
    ...
    it('should not count duplicates', async () => {
      await client.pfadd(testKey, 'element1', 'element1', 'element2', 'element2');
      const count = await client.pfcount(testKey);
    ...
    it('should provide approximate count for large datasets', async () => {
      ...
      assert.ok(Math.abs(count - expectedCount) < errorMargin, 
  ...
  describe('Error Handling', () => {
    it('should handle wrong type operations', async () => {
      ...
      try {
        ...
        assert.ok(err);
        assert.ok(err.message.includes('WRONGTYPE') || err.message.includes('wrong'));
      }
    });
